...
void windowSelection::resize(int viewX, int viewY)
{
	...
	_info.size = D3DXVECTOR2(
		(_bindWindow->getInfo().size.x - BTN_SCROLL_SIZE_X) / _viewCount.x,
		(_bindWindow->getInfo().size.y - BTN_MOVE_BAR_SIZE_Y) / _viewCount.y);
}
...
UI_LIST_NODE windowSelection::updateWindow(void)
{
	...
	_info.pos.y = (*_bindIndex / (int)_viewCount.y) * _info.size.y - *_bindOffset + BTN_MOVE_BAR_SIZE_Y;
